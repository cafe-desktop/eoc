...
struct _EocMetadataReaderPngPrivate {
	EocMetadataReaderPngState  state;
...
G_DEFINE_TYPE_WITH_CODE (EocMetadataReaderPng, eoc_metadata_reader_png,
			 G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (EOC_TYPE_METADATA_READER,
			                        eoc_metadata_reader_png_init_emr_iface) \
			                        G_ADD_PRIVATE(EocMetadataReaderPng))

static void
eoc_metadata_reader_png_dispose (GObject *object)
{
	EocMetadataReaderPng *emr = EOC_METADATA_READER_PNG (object);
	EocMetadataReaderPngPrivate *priv = emr->priv;
	...
	G_OBJECT_CLASS (eoc_metadata_reader_png_parent_class)->dispose (object);
}

static void
eoc_metadata_reader_png_init (EocMetadataReaderPng *emr)
{
	EocMetadataReaderPngPrivate *priv;

	priv = emr->priv =  eoc_metadata_reader_png_get_instance_private (emr);
	priv->icc_chunk = NULL;
	priv->icc_len = 0;
	priv->xmp_chunk = NULL;
	priv->xmp_len = 0;
...
static void
eoc_metadata_reader_png_class_init (EocMetadataReaderPngClass *klass)
{
	GObjectClass *object_class = (GObjectClass*) klass;

	object_class->dispose = eoc_metadata_reader_png_dispose;
}

static gboolean
eoc_metadata_reader_png_finished (EocMetadataReaderPng *emr)
{
...
static void
eoc_metadata_reader_png_get_next_block (EocMetadataReaderPngPrivate* priv,
				    	guchar *chunk,
					int* i,
					const guchar *buf,
					int len,
...
static void
eoc_metadata_reader_png_consume (EocMetadataReaderPng *emr, const guchar *buf, guint len)
{
	EocMetadataReaderPngPrivate *priv;
 	int i;
	guint32 chunk_crc;
	...
	for (i = 0; (i < len) && (priv->state != EMR_FINISHED); i++) {
		...
		case EMR_CHECK_CRC:
			...
			eoc_debug_message (DEBUG_IMAGE_DATA, "Checking CRC: Chunk: 0x%X - Target: 0x%X", chunk_crc, priv->target_crc);
		...
		case EMR_READ_XMP_ITXT:
			...
			eoc_metadata_reader_png_get_next_block (priv,
							    priv->xmp_chunk,
							    &i, buf, len,
							    EMR_READ_XMP_ITXT);
...
static gpointer
eoc_metadata_reader_png_get_xmp_data (EocMetadataReaderPng *emr )
{
	EocMetadataReaderPngPrivate *priv;
	XmpPtr xmp = NULL;
...
static gpointer
eoc_metadata_reader_png_get_icc_profile (EocMetadataReaderPng *emr)
{
	EocMetadataReaderPngPrivate *priv;
	...
	if (priv->icc_chunk) {
		...
		do {
			if (zstr.avail_out == 0) {
				...
				if (G_UNLIKELY (new_size > EOC_ICC_INFLATE_BUFFER_LIMIT)) {
					...
					inflateEnd (&zstr);
					g_free (outbuf);
					eoc_debug_message (DEBUG_IMAGE_DATA, "ICC profile is too large. Ignoring.");
					return NULL;
				}
		...
		if (G_UNLIKELY (z_ret != Z_STREAM_END)) {
			eoc_debug_message (DEBUG_IMAGE_DATA, "Error while inflating ICC profile: %s (%d)", zstr.msg, z_ret);
			inflateEnd (&zstr);
			g_free (outbuf);
			return NULL;
		}
		...
		eoc_debug_message (DEBUG_LCMS, "PNG has %s ICC profile", profile ? "valid" : "invalid");
	}

	if (!profile && priv->sRGB_chunk) {
		eoc_debug_message (DEBUG_LCMS, "PNG is sRGB");
	...
	if (!profile && priv->cHRM_chunk && priv->gAMA_chunk) {
		...
		eoc_debug_message (DEBUG_LCMS, "Trying to calculate color profile");
		...
		eoc_debug_message (DEBUG_LCMS, "Gamma %.5lf", gammaValue);
...
eoc_metadata_reader_png_init_emr_iface (gpointer g_iface, gpointer iface_data)
{
	...
	iface->consume =
		(void (*) (EocMetadataReader *self, const guchar *buf, guint len))
			eoc_metadata_reader_png_consume;
	iface->finished =
	...
	iface->get_xmp_ptr =
		(gpointer (*) (EocMetadataReader *self))
			eoc_metadata_reader_png_get_xmp_data;
#endif
}
